...
def importCSVPersons(context, path, file):
    ...
    for person in results:
        ...
        if organization and len(organization)>1:
            aux = _('There are ${org_size} organizations with the same title, i don\'t know with which one relate this person.',\
                    mapping={'org_size':len(organization)})

            msg = zope.i18n.translate(aux, context=context.request)
            LOG(PROJECTNAME, WARNING, msg)

            aux = _('You will have to load the person with id "${person_id}" manually.',\
            ...
            aux = _('There\'s no organization with title ${organization_name}, make sure it exists before adding persons. "${person_id}" not added',\
...
def importCSVOrganizations(context, path, file):
    ...
    for organization in results:
        if context.get(organization['id']):
            ...
            aux = _('You will have to load the organization with id "${organization_id}" manually.',\
