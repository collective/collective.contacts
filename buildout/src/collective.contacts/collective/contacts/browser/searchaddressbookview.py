...
class SearchAddressBookView(BrowserView):
    ...
    pt = ViewPageTemplateFile('./searchaddressbookview.pt')
    ...
    def __call__(self):
        ...
        if mailto or export_persons or export_organizations:
            ...
            if not form.has_key('user_selection'):
                status = _(u'You need to select at least one person or organization')
        ...
        if mailto_group:
            if not form.has_key('user_selection'):
                status = _(u'You need to select at least one group')
            ...
            if self.request.string_emails == '':
                status = _(u'There are no persons to send an email to')
    ...
    def quick_search_organizations(self, request):
        ...
        all_organizations = [i.getObject() for i in self.portal_catalog(
                             {'portal_type':'Organization',
                              'path':'/'.join(self.context.getPhysicalPath()),
                              'SearchableText':request.get('search_string', ''),
            ...
            for attr in attrs.keys():
                if attrs[attr] and attrs[attr] != '':
                    ...
                    if attr == 'organization':
                        person_data = str(getattr(person, attr, '').title).\
                                      lower()
                    else:
